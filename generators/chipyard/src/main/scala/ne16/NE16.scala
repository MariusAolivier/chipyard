...
class NE16TL(params: NE16Params, beatBytes: Int)(implicit p: Parameters)
  ...
  class NE16TLImpl extends Impl {
    withClockAndReset(clock, reset) {
      ...
      // HWPE peripheral semantics use wen=1 for reads and wen=0 for writes.
      ...
      when(scratchpad.a.fire && scratchpadTrace =/= 255.U) {
        printf(p"NE16 scratchpad A fire addr=0x${Hexadecimal(scratchpad.a.bits.address)} hasData=$scratchpadHasData\n")
