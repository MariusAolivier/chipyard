...
static inline void load_weights_prepare(Layer tile, Kernel kernel,
    ...
    const int size_weights = (kernel.groups > 1 ? divnceil(tile.output.channel, 16) : tile.output.channel) * 144;
    const int size_scale = tile.output.channel * 4;
    const int size_bias = tile.output.channel * 4;
    ...
    if (pi_core_id() == LOADER_ID) {
        ...
        Layer layer = {
            .addr = {
                ...
                .scale = layer_args->L2_weights + 2304,
                .bias = layer_args->L2_weights + 2368,
                .output = layer_args->L2_output
            },
            .padding = {
                .top    = layer_args->padding & NET_UTILS_PAD_TOP ? 1 : NE16_DONT_PAD,
                .right  = 1,
                .bottom = layer_args->padding & NET_UTILS_PAD_BOTTOM ? 1 : NE16_DONT_PAD,
                .left   = 1
            }
        };
        ...
        const int l1_buffer_input = l1_buffer + 0;
        const int l1_buffer_output = l1_buffer + 1024;
        const int l1_buffer_weights = l1_buffer + 2048;
        const int l1_buffer_scale = l1_buffer + 4352;
        const int l1_buffer_bias = l1_buffer + 4416;
        ...
        Address buffer_addresses[2] = {
            ...
            {
                .input = l1_buffer_input + 1024,
                .weights = l1_buffer_weights + 2304,
                .scale = l1_buffer_scale + 64,
                .bias = l1_buffer_bias + 64,
                .output = l1_buffer_output + 1024
            }
        };
...
void BNReluConvolution0(void *args) {
